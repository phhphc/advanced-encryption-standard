...
def blockEncrypt(k, m):
    ...
    def shiftRows(m):
        shift = ( 0x0, 0x5, 0xA, 0xF, 0x4, 0x9, 0xE, 0x3, 0x8, 0xD, 0x2, 0x7, 0xC, 0x1, 0x6, 0xB )
    ...
    def mixColumns(m):
        ...
        r = [None]*16
        for i in range(4):
            [r[4*i], r[4*i + 1], r[4*i+2], r[4*i+3]] = mix_column([m[i*4 + j] for j in range(4)])
        return r

    r = len(k) - 1
    c = xor(m, k[0])
    for i in range(1,r):
        c = xor(k[i], mixColumns(shiftRows(subBytes(c))))
...
def blockDecrypt(k, c):
    ...
    def invMixColumns(m): 
        def inv_mix_column(col):
            return [
                GF_Mul_14[col[0]] ^ GF_Mul_11[col[1]] ^ GF_Mul_13[col[2]] ^ GF_Mul_9[col[3]],
                GF_Mul_9[col[0]]  ^ GF_Mul_14[col[1]] ^ GF_Mul_11[col[2]] ^ GF_Mul_13[col[3]],
                GF_Mul_13[col[0]] ^ GF_Mul_9[col[1]]  ^ GF_Mul_14[col[2]] ^ GF_Mul_11[col[3]],
                GF_Mul_11[col[0]] ^ GF_Mul_13[col[1]] ^ GF_Mul_9[col[2]]  ^ GF_Mul_14[col[3]]
            ]
    ...
    r = len(k) - 1
    c = invSubBytes(invShiftRows(xor(c, k[r])))
